...
func TestCommandRunBarosaAzure(t *testing.T) {
	...
	if err == nil {
		t.Fatalf("Expected fail when giving filename to send to azure")
	}
}
...
func TestCommandRunBarosaScreenshot(t *testing.T) {
	...
	if err == nil {
		t.Fatalf("Expected error when giving window class")
	}

	_, err = command.CommandRunBarosaScreenshot("plasma", "class")
	defer os.Remove("plasma_screenshot")
	if err != nil {
		t.Fatalf("Expected success screenshotting plasma window class.. (%v)", err)
	}

	_, err = command.CommandRunBarosaScreenshot("plasma", "METHOD4")
	if err == nil {
		t.Fatalf("Expected error when giving method")
	}
}
